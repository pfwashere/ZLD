...
public class Simulation1 : MonoBehaviour
{
    ...
    private float energyProduce = 0f;
    public bool isWaterOn = true;
    ...
    void Update()
    {
        ...
        {
            lossRateText.text = $"Loss Rate: {lossRate:F2} L/s\nFlow Rate: {flowRate:F2} L/s";
        }
        ...
        if (isWaterOn)
        {
            ...
            energyconsumption = Mathf.Clamp(energyconsumption, 68f, 79f);

            float energyProduceChange = Random.Range(-0.2f, 0.2f);
            energyProduce += energyProduceChange;
            energyProduce = Mathf.Clamp(energyProduce, 68f, 79f);
        }
        else
        {
            // น้ำปิด → อุณหภูมิลดลงเหลือ 0°C อย่างช้าๆ
            temperature = Mathf.MoveTowards(temperature, 0f, Time.deltaTime * 5f); // ปรับความเร็วได้

            // ถ้าปิดน้ำ ให้ลด Energy Consumption ลงเรื่อยๆ จนเป็น 0
            energyconsumption = Mathf.MoveTowards(energyconsumption, 0f, Time.deltaTime * 20f);

            // ถ้าปิดน้ำ ให้ลด Energy Produce ลงเรื่อยๆ จนเป็น 0
            energyProduce = Mathf.MoveTowards(energyProduce, 0f, Time.deltaTime * 20f);
        }
        ...
        {
            EnergyConsumpTionText.text = $"Energy Consumption: {energyconsumption:F1} kWh";
        }

        if (EnergyProduceText != null)
        {
            EnergyProduceText.text = $"Energy Produce: {energyProduce:F1} kWh";
        }
